...
import { ProjectItem, SkillCategory, YouTubeTutorial, Review } from "./types";
...
const CommunityContributionsSection: React.FC = () => {
  ...
  const profileUrl =
    c?.profileUrl ||
    "https://www.skool.com/@elvis-eyobor-3582?g=learn-power-apps";
  const imageUrl =
    c?.imageUrl ||
    "https://firebasestorage.googleapis.com/v0/b/studio-1523050100-2a1a4.firebasestorage.app/o/Elvis%20Portfolio%20Files%2FSkool%20Profile%20(1).jpeg?alt=media&token=b5218e4c-aea6-417e-a330-d3e7b87647bd";
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          <div className="lg:col-span-5 space-y-6 order-2 lg:order-2">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-lg bg-blue-50 flex items-center justify-center text-[#1E40AF]">
            ...
            <p className="italic text-slate-500">
              Empowering the Power Platform Ecosystem Through Active Support and
              Knowledge Sharing
            </p>

            <p className="text-lg text-slate-700 max-w-3xl">
              I am a dedicated contributor to the Power Platform community on{" "}
              {c?.platform || "Skool"}, a dynamic network of approximately{" "}
              {c?.members?.toLocaleString() || "3,000"} members.
            </p>

            <p className="text-lg text-slate-700 max-w-3xl">
              With over {c?.contributions || "600"} contributions—including
              expert support responses, insightful posts, and collaborative
              discussions—I help developers and professionals troubleshoot
              challenges, share best practices, and drive innovation in tools
              like Power Apps, Power Automate, and Power BI.
            </p>
...
const YouTubeTutorialsSection: React.FC<{ tutorials: YouTubeTutorial[] }> = ({
  tutorials,
}) => {
      ...
      <div className="container mx-auto px-6">
        <div className="flex justify-between items-end mb-16">
          <div className="space-y-4">
            <h2 className="text-sm font-black text-slate-400 uppercase tracking-[0.4em]">
              Learn
            </h2>
            <h3 className="text-5xl font-extrabold tracking-tighter text-slate-900">
              Top Tutorials
            </h3>
          </div>
          <a
          ...
          {tutorials.map((tutorial, idx) => (
            ...
            >
              <div className="aspect-video bg-slate-200 rounded-3xl overflow-hidden relative shadow-sm transition-all hover:shadow-2xl">
                ...
                <div
                  className={`absolute inset-0 transition-all flex items-center justify-center ${hoveredIdx === idx || playingId === tutorial.id ? "bg-black/10" : "bg-black/0 group-hover:bg-black/20"}`}
                >
                  <div
                    className={`w-16 h-16 bg-white/90 rounded-full flex items-center justify-center transition-all ${hoveredIdx === idx || playingId === tutorial.id ? "opacity-100 scale-100" : "opacity-0 scale-50"}`}
                  >
            ...
            <div className="w-full max-w-4xl bg-transparent relative rounded-lg overflow-hidden">
              ...
              <div className="aspect-video w-full bg-black">
                <iframe
                  ...
                  title={
                    tutorials.find((t) => t.id === playingId)?.title || "Video"
                  }
                  className="w-full h-full border-0"
                  allow="autoplay; encrypted-media; picture-in-picture"
                  allowFullScreen
...
// Reviews are handled by the standalone ReviewSection component.
...
const Navbar: React.FC<{
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 transition-all duration-500 ${isScrolled ? "py-2" : "py-6"}`}
    >
      <div className="container mx-auto px-6">
        <div
          className={`flex justify-center items-center px-8 py-4 rounded-3xl transition-all ${isScrolled ? "glass shadow-2xl border-slate-200/50" : "bg-transparent"}`}
        >
      ...
      {/* Mobile Menu */}
      <div
        className={`md:hidden fixed inset-0 z-40 bg-white/95 backdrop-blur-2xl transition-transform duration-500 transform ${mobileMenuOpen ? "translate-x-0" : "translate-x-full"}`}
      >
...
const SocialMediaLinks: React.FC<{ containerClasses?: string }> = ({
  containerClasses = "",
}) => {
  return (
    <div className={containerClasses}>
      {portfolioData.socialMedia.map((social) => (
        ...
        >
          <img
            className="w-14 h-14 flex items-center justify-center w-6 h-6"
            src={social.iconUrl}
            alt={social.name}
          />
          <span className="ml-2 text-sm font-bold invisible opacity-0 group-hover:visible group-hover:opacity-100 transition-opacity text-[#742774]">
            {social.name}
          </span>
        </a>
      ))}
...
const LandingScreen: React.FC<{
  onNavigate: (s: Screen, d?: any) => void;
  featuredProjects: ProjectItem[];
  youtubeTutorials: YouTubeTutorial[];
}> = ({ onNavigate, featuredProjects, youtubeTutorials }) => {
  const subheadlines = [
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-4xl mx-auto text-center space-y-8">
            <h2 className="text-6xl md:text-8xl font-black tracking-tighter text-slate-900 leading-[0.85]">
              ...
              <span className="text-[#742774]">Code & Data.</span>
            </h2>
            <p className="text-2xl text-slate-500 leading-relaxed font-medium max-w-3xl mx-auto">
              As a Microsoft Certified Power Platform Developer, I transform
              complex business challenges into streamlined, data-driven
              solutions. With expertise in Power Apps, Power Automate, and Power
              BI, I build efficient workflows and interactive dashboards that
              drive real results. My background in maintenance planning adds a
              unique edge to optimizing operations across industries...
            </p>
            <button
              onClick={() => onNavigate("about")}
              className="bg-[#742774] text-white px-8 py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:opacity-90 transition-all shadow-lg shadow-purple-100"
            >
              Read More About Me
            </button>
          </div>
        ...
        <div className="container mx-auto px-6">
          <div className="flex justify-between items-end mb-16">
            ...
            <button
              ...
              className="hidden md:flex items-center gap-3 text-sm font-black uppercase tracking-widest text-slate-400 hover:text-[#742774] transition-colors"
            >
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {featuredProjects.map((p, idx) => {
              const pics = Array.isArray(p.project_pictures)
                ? p.project_pictures
                : [];
              ...
              if (p.name === "Vendor Management System") {
                return (
                  ...
                  >
                    ...
                    <div className="px-2">
                      <h4 className="text-2xl font-black tracking-tighter text-slate-900 mb-2 group-hover:text-[#742774] transition-colors">
                        {p.name}
                      </h4>
          ...
          <div className="mt-16 flex justify-center">
            <button
              onClick={() => onNavigate("projects")}
              className="bg-slate-900 text-white px-10 py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-[#742774] transition-all"
            >
              Show More Projects
            </button>
          </div>
...
const AboutScreen: React.FC<{ onNavigate: (s: Screen) => void }> = ({
  onNavigate,
}) => {
  return (
        ...
        <div className="grid lg:grid-cols-12 gap-16 items-start">
          ...
          <div className="lg:col-span-7 space-y-16">
            ...
            <section className="space-y-12">
              <h3 className="text-sm font-black uppercase tracking-[0.4em] text-slate-400">
                Core Expertise
              </h3>
                ...
                {portfolioData.technicalSkills.map((cat, idx) => (
                  <div key={idx} className="space-y-6">
                    <h4 className="text-xl font-black tracking-tight flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-purple-50 flex items-center justify-center text-[#742774]">
                        <Layers size={16} />
                      </div>
                      {cat.category}
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {cat.skills.map((s, i) => (
                        <span
                          key={i}
                          className="px-4 py-2 bg-slate-50 rounded-xl text-xs font-bold border border-slate-100"
                        >
                          {s}
                        </span>
                      ))}
                ...
                {portfolioData.experience.map((exp, idx) => (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <div className="text-[10px] font-black uppercase tracking-widest text-[#f09d1b]">
                        {exp.period}
                      </div>
      ...
      <div className="container mx-auto px-6">
        <div className="mb-20 space-y-4">
          <h1 className="text-6xl md:text-8xl font-black tracking-tighter text-slate-900">
            Projects<span className="text-[#742774]">.</span>
          </h1>
          <p className="text-xl text-slate-500 font-medium max-w-xl">
            A comprehensive list of enterprise solutions developed across
            various industries.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, idx) => {
            const pics = Array.isArray(project.project_pictures)
            ...
            return (
              ...
              >
                ...
                <div className="px-2">
                  <div className="text-[10px] font-black uppercase tracking-[0.2em] text-[#742774] mb-2">
                    {project.client || project.company || "Enterprise"}
                  </div>
                  <h4 className="text-2xl font-black tracking-tighter text-slate-900 leading-tight group-hover:text-[#742774] transition-colors">
                    {project.name}
                  </h4>
                </div>
              </div>
  ...
  return (
    <div className="pt-32 pb-32 animate-in fade-in zoom-in-95 duration-500">
      <div className="container mx-auto px-6 max-w-3xl">
        <button
          onClick={onBack}
          className="text-xs font-black uppercase tracking-widest text-[#742774] flex items-center gap-2 mb-12"
        >
          <ChevronRight size={14} className="rotate-180" /> Back to projects
        </button>
        <div className="space-y-12">
          <div className="space-y-4">
            <div className="text-xs font-black uppercase tracking-[0.3em] text-[#f09d1b]">
              {project.period || "Case Study"}
            </div>
            <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-slate-900 leading-[0.9]">
              {project.name}
            </h1>
            <div className="px-4 py-2 inline-block bg-slate-50 rounded-xl text-xs font-bold border border-slate-100">
              {project.client || project.company || "Confidential"}
            </div>
          </div>
          ...
          {restPics.length > 0 && (
            <div className="grid grid-cols-1 gap-6">
              {restPics.map((url, idx) => (
                <div
                  key={idx}
                  className="rounded-3xl overflow-hidden bg-slate-50 border border-slate-100 shadow-lg"
                >
                  <img
                    src={url}
                    className="w-full h-auto object-cover"
          ...
          <div className="p-10 bg-slate-50 rounded-[2.5rem] border border-slate-100">
            <h4 className="text-[10px] font-black uppercase tracking-[0.3em] mb-6 text-slate-400">
              Tech Stack
            </h4>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://formspree.io/f/mqaebrjr", {
        // Replaced with a real generic endpoint logic
        method: "POST",
            ...
            <div className="relative z-10 grid lg:grid-cols-2 gap-20 items-center">
              ...
              <div className="space-y-8 text-white">
                ...
                <p className="text-xl text-white/70 font-medium max-w-sm">
                  I'm ready to help you transform your business workflows into
                  competitive advantages.
                </p>
                <div className="flex gap-4">
                  <a
                    href="#"
                    className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center hover:bg-white hover:text-[#742774] transition-all border border-white/20"
                  >
                    <Linkedin size={24} />
                  </a>
                  <a
                  ...
                  {formState === "success" ? (
                    <div className="text-center py-20 space-y-4">
                      ...
                      <button
                        ...
                        className="text-[#742774] font-black text-xs uppercase tracking-widest mt-4"
                      >
                        Send another
                      </button>
                    </div>
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setReviews(portfolioData.reviews || []);
    ...
    <div className="min-h-screen flex flex-col selection:bg-[#742774] selection:text-white">
      ...
      <main className="flex-grow">
        ...
        {currentScreen === "projects" && (
          <ProjectsScreen projects={projects} onNavigate={navigate} />
        )}
      ...
      <footer className="py-20 border-t border-slate-100 bg-white">
        <div className="container mx-auto px-6 text-center space-y-6">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-300">
            © 2024 Power Platform Solutions Specialist
          </p>
        </div>
      </footer>
    </div>
